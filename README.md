...
| [`vtuber-worker-node`](./vtuber-worker-node) | `vtuber-worker-node/` | Payee-side worker for the **`livepeer:vtuber-session`** capability. Hosts the `StreamingModule` interface; validates and debits receiver-side session balance via co-located `payment-daemon`; forwards session-open requests to a local `session-runner` backend (housed in `livepeer-vtuber-project`). Go binary, custom `payment-middleware-check` golangci-lint analyzer | `main` | `v4.0.1` (`633049f`) |
