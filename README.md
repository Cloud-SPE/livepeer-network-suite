...
|---|---|---|---|---|
| [`livepeer-modules`](./livepeer-modules) | `livepeer-modules/` | On-chain control plane: `chain-commons` lib + `payment-daemon`, `service-registry-daemon`, `protocol-daemon` (gRPC over unix sockets, Arbitrum One) | `main` | `v2.1.0-4` (`8232880`) |
...
| [`livepeer-orch-coordinator`](./livepeer-orch-coordinator) | `livepeer-orch-coordinator/` | Public-facing fleet dashboard + signed-manifest hosting at the on-chain `serviceURI`. **No keys, no daemon sockets.** Verifies sig against on-chain orch identity. Worker Prometheus scraping; daily-ops console | `main` | `v3.0.0` (`0043d64`) |
